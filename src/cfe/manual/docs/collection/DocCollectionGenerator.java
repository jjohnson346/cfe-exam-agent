...
public class DocCollectionGenerator {
	...
	public DocCollectionGenerator(CFEManual cfeManual) {
		...
		cfeManualClassName = StringUtils.substringAfterLast(cfeManualFullClassName, ".");
	}
	...
	//
	// File cfeManualDir = new File("document collection/" + cfeManualClassName);
	// // if doc collection already exists, return.
	// if (cfeManualDir.exists()) {
	// System.out.println("Document collection already exists for " + cfeManualClassName);
	// return;
	// }
	//
	...
	// File sectionDir = new File("document collection/" + cfeManualClassName + "/" + root.id);
	// sectionDir.mkdir();
	//
	// File sectionFile = new File("document collection/" + cfeManualClassName + "/" + root.id + "/" + root.name
	// + ".txt");
	// sectionFile.createNewFile();
	// outputSectionToFile(root, sectionFile);
	//
	// for (CFEManualSection section : root.subSections.values()) {
	// File subSectionDir = new File("document collection/" + cfeManualClassName + "/" + root.id + "/"
	// + section.name);
	//
	// subSectionDir.mkdir();
	//
	// File subSectionFile = new File("document collection/" + cfeManualClassName + "/" + root.id + "/"
	...
	// // cfeManualFullClassName.substring(cfeManualFullClassName.lastIndexOf(".") + 1);
	//
	// File docCollectionDir = new File("document collection");
	// if (!docCollectionDir.exists())
	// docCollectionDir.mkdir();
	...
	// CFEManualSection root = cfeManual.getRoot();
	//
	// File sectionFile = new File("document collection/" + cfeManualClassName + "/" + docIdAsString(4, 1) + "-"
	// + root.name + ".txt");
	// sectionFile.createNewFile();
	// outputSectionToFile(root, sectionFile);
	//
	// int i = 1;
	// for (CFEManualSection section : root.subSections.values()) {
	// File subSectionFile = new File("document collection/" + cfeManualClassName + "/"
	...
	private int createDocument(int docId, int depth, CFEManualSection section) throws IOException {
		// first, make sure the file name and question section name consists of only valid characters.
		String nameOfFileForSection = section.name.replace(": ", " - ").replace("/", "-");
		...
		try {
			...
			examSectionDirName = "document collection/" + cfeManualClassName + "/" + section.examSection;
			...
			questionSectionDirName = examSectionDirName + "/" + section.questionSection;
			File questionSectionDir = new File(questionSectionDirName);
			if (!questionSectionDir.exists())
				questionSectionDir.mkdir();

			// create document file.
			documentFileName = questionSectionDirName + "/" + docIdAsString(4, docId) + StringUtils.repeat("-", depth)
	...
	private String buildDocumentText(CFEManualSection section) {
		...
		// sb.append(String.format("%s%s\n\n", "Exam Section=", section.examSection));
		// sb.append(String.format("%s%s\n\n", "Question Section=", section.questionSection));
		// sb.append(String.format("%s%s\n\n", "Raw Text=", section.getText()));
		// sb.append(String.format("%s%s\n\n", "Stemmed Text=", stemSectionText(section)));
...
//	}
	 ...
	 *            the section whose text is to be output to the file
	 */
	private void outputSectionToJSONFile(CFEManualSection section, File sectionFile) {
		try {
...
//	}
	...
	public void cleanDocCollection() {
		String cfeManualFullClassName = cfeManual.getClass().getName();
		String cfeManualClassName = cfeManualFullClassName.substring(cfeManualFullClassName.lastIndexOf(".") + 1);
		File cfeManualDir = new File("document collection/" + cfeManualClassName);
		try {
			delete(cfeManualDir);
		} catch (Exception e) {
	...
	public static String docIdAsString(int targetLength, int docId) {
		if (docId == 0) {
			...
			int docIdLength = (int) Math.log10(docId) + 1; // stores the length of docId
			return StringUtils.repeat("0", targetLength - docIdLength) + String.valueOf(docId);
		}
	}

	public static void main(String[] args) {
		// DocCollectionGenerator dcu = new DocCollectionGenerator(new CFEManualSmallDocUnit());
		DocCollectionGenerator dcu = new DocCollectionGenerator(new CFEManualLargeDocUnit());
