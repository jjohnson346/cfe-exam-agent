...
/**
 * MaxFrequency is an algorithm that returns the option which occurs most
 * frequently in the question section of the manual. The counts are based on
 * instances of the complete option phrase in the manual. No further
 * sophistication, (partial instances, stemming, etc.) is applied in this
 * algorithm - not even functionality to account for "none of the above" or "all
 * of the above".
 * 
 * @author Joe
 *
...
public class MaxFrequency implements IAlgorithmFrequency {
	...
	// TODO: eliminate options all of the above, none of the above from option
	// counts.
	// Considering these in the computation could throw off results relative to
	// min/max
	// thresholds.

	// version 2.0.0 - removed CFEManualLargeDocUnit variable
	// protected CFEManualLargeDocUnit cfeManual; // the cfe manual object

	protected CFEExamQuestion question; // the question to apply the algorithm
										// to

	protected ArrayList<Double> optionFrequencies; // frequency counts of the
													// options
	 ...
	 * @param cfeManual
	 */
	// public MaxFrequency(CFEManual cfeManual) {
	...
	public int solve(CFEExamQuestion question, CFEManual cfeManual) {
		// TODO: include logic for addressing options: all of the above, none of
		// the above.
		...
		// retrieve the relevant text in the manual for the section specified by
		// the question.
		Logger.getInstance().printf(DetailLevel.FULL, "%s%s\n",
				"Retrieving section text for question section, ",
				question.section);

		// 09/13/2014 - changed the code here to use CFEManualSection
		CFEManualSection section = cfeManual
				.getManualSectionForQuestionSection(question.section);
		String sectionText = section.getText();
		// String sectionText =
		// cfeManual.getQuestionSectionText(question.section);
		...
		for (String phrase : question.options) {
			...
			// note that there's currently no stemming here and no accounting
			// for
	...
	public void printOptionFrequencies() {
		...
		Logger.getInstance().printf(DetailLevel.FULL,
				"%" + maxPhraseLength + "s%20s\n", "Phrase", "Frequency");
		for (int i = 0; i < question.options.size(); i++) {
			Logger.getInstance().printf(DetailLevel.FULL,
					"%" + maxPhraseLength + "s%19.3f\n",
	...
	/**
	 * returns the number of occurrences of an option phrase within the section
	 * of text. This method does no stemming and does not address the situation
	 * when the option is "all of the above" or "none of the above".
	 * 
	 * @param optionPhrase
	 *            the phrase to search for in the text
	 * @param sectionText
	 *            the text within which to find the number of occurrences of
	 *            optionPhrase
	...
	protected int getPhraseCount(String optionPhrase, String sectionText) {
		...
		int count = 0;
		do {
			lastIndex = sectionText.toLowerCase().indexOf(
					optionPhrase.toLowerCase(), lastIndex);
			if (lastIndex != -1) {
				count++;
