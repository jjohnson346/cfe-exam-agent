...
/**
 * MLPassage2 uses a bag of words/bayesian algorithm to determine the relative likelihood of each option given the
 ...
 * 
 * Unlike MLPassage1, this algorithm uses only the first of the 7 passages. It was noticed that most of the time the
 * passages, delivered in decreasing order of log-regression probability, are such that the first/top passage is
 * actually the correct passage, so this algorithm leverages that tendency. To be clear, however, this algorithm does
 * NOT use the field, isCorrectPassage (for which the isCorrectPassage field is often true for top passages.)
 * 
 */
public class MLPassage2Old implements IAlgorithm {

	private final String INPUT_FILE = "ml.test.7.txt";

	@Override
	public int solve(CFEExamQuestion question, CFEManual cfeManual) {
		System.out.println("executing mlpassage algorithm for " + question.name + "...");
		try {
			...
			for (String optionPhrase : question.options) {
				...
				for (String optionToken : optionTokens) {
					if (passagesWordCounts.containsKey(optionToken)) {
						int optionTokenTypeFreq = passagesWordCounts.get(optionToken);
	 ...
	 * downstream to calculate word frequencies. Note - also converts the words to lower case.
	 * 
	 * @param passages
	...
	/**
	 * extracts the sequence of words from a text string as a list. This list is used downstream to calculate word
	 * frequencies. Note - also converts the words to lower case.
	 * 
	 * @param text
