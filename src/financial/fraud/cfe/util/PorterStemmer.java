 ...
 * word can be provided a character at time (by calling add()), or at once by 
 * calling one of the various stem(something) methods.
 * 
 * The main() method of this class performs a unit test of this class, accepting
 * a list of words in an input file, and writing its results to the standard
 * output console.  Run this method to get an idea of how to use it this class.
 */
...
public class PorterStemmer {
	...
	/**
	 * Clears the buffer, essentially re-starting the PorterStemmer instance. Good for when you want to just stem one word at a time
	 */
	public void clear() {
		b = new char[INC];
		i = 0;
	...
	/**
	 * Stem the word placed into the PorterStemmer buffer through calls to add(). Returns true if the stemming process resulted in a word different from the
	 * input. You can retrieve the result with getResultLength()/getResultBuffer() or toString().
	 */
	public void stem() {
		k = i - 1;
		if (k > 1) {
	...
	/**
	 * Test program for demonstrating the PorterStemmer. It reads text from a a list of files, stems each word, and writes the result to standard output. Note
	 * that the word stemmed is expected to be in lower case: forcing lower case must be done outside the Stemmer class. Usage: Stemmer file-name file-name ...
	 */
