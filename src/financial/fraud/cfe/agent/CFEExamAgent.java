...
/**
 * An instance of CFEExamAgent represents an agent taking a cfe exam.  
 * This is the entry point class for the cfe exam agent software, in particular,
 * the main method, which instantiates the CFExamAgent and executes the takeExam() 
 * method.
 * 
 * @author jjohnson346
 *
...
public class CFEExamAgent {
	private CFEManualLargeDocUnit cfeManual;			// the cfe manual object that the agent will use to study
										// up for the test.  That is, the agent initializes this 
										// reference in its own constructor to a new instance of the 
										// manual.

	private CFEExam cfeExam;				// a reference to an exam object that represents the exam
										// the agent will take

	private IAlgorithm[] algos;			// an array containing all of the algorithms from which the
										// the agent can select the most believed to be the most appropriate
										// one for answering each question

	private String executionMode;		// batch or interactive?

	private ProfileData profileData;		// stores the probabilities of success for each question profile,
										// for each algorithm

	
	/**
	 * constructor that initializes algos array, loads profile data (probs of success by algo, 
	 * and question profile), and sets execution mode.
	 * 
	 * @param executionMode 		sets whether to run in batch or interactive mode
	 */
	public CFEExamAgent(String executionMode) {
		Logger.getInstance().println("Exam Agent initializing...", DetailLevel.MINIMAL);
		...
		try {
			...
			Logger.getInstance().println("CFE Exam Agent initialization complete.", DetailLevel.MINIMAL);
		} catch (FileNotFoundException e) {
			for (StackTraceElement s : e.getStackTrace()) {
				Logger.getInstance().printf(DetailLevel.NONE, "%s", s);
			}
			Logger.getInstance().println("CFE Exam Agent terminated unsuccessfully.", DetailLevel.NONE);
		}
		
							}

	/**
	 * initiates the taking of the test by the cfe agent.  This is the primary method 
	 * for the entire program.
	 * 
	 * @param examName 		the name of the exam from which the exam file name will be determined
	 */
...
//			outputWriter.write("%s\n", cfeExam.getFormattedQuestion(i));
			...
			response = bestAlgo.solve(question);

			Logger.getInstance().println("Algorithm selected: " + bestAlgo.toString(), DetailLevel.MEDIUM);
//			System.out.println("Algorithm selected: " + bestAlgo.toString());
			...
			if (response == question.correctResponse) {
				...
				result = "INCORRECT. (Correct answer:  " + question.getFormattedCorrectResponse() + ")";
			}

			Logger.getInstance().printf(DetailLevel.MINIMAL, "%s%s%s%s\n\n", "Agent response:  ", question.getFormattedResponse(response), " -- ", result);
//			outputWriter.write("%s%s%s%s\n\n", "Agent response:  ", question.getFormattedResponse(response), " -- ", result);

//			if (executionMode.equals("Interactive") && bestAlgo instanceof IAlgorithmFrequency) {
//				
//				// prompt user whether to output section to file. If yes, send it to output stream.
...
//			}
	...
	/**
	 * selects the algorithm to use for a particular question, based on the profile data
	 * and the profile for the question.
	 * 
	 * @param question
	 * @return
	 */
	private IAlgorithm selectAlgorithm(CFEExamQuestion question) {
		...
		Logger.getInstance().println("Question profile index: " + profileIndex, DetailLevel.MEDIUM);
		...
		// question profile.  Thus, the optimal algorithm should be a hash map lookup 
	...
	/**
	 * the entry point to the cfe exam agent software.  instantiate
	 * a CFEExamAgent and kick off the takeExam() method.
	 * 
	 * @param args
	 */
	public static void main(String[] args) {
		...
		logger.addDestination("logs//cfe exam agent " + appConfig.EXAM_NAME + ".log");
		// logger.addDestination("logs\\cfe exam agent " + appConfig.EXAM_NAME + ".log");
